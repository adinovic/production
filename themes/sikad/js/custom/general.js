...
jQuery(document).ready(function(){
	...
	jQuery('.userinfo3').click(function(){
		if(!jQuery(this).hasClass('active')) {
			jQuery('.userinfodrop3').show();
			jQuery(this).addClass('active');
		} else {
			jQuery('.userinfodrop3').hide();
			jQuery(this).removeClass('active');
		}
		//remove notification box if visible
		jQuery('.notification').removeClass('active');
...
//////////////// AUTO GENERATE BOOTSTRAP 3 TABLE 7 DELETED UNNECESSARY TABLE STYLES /////////////////////////////

// oTables = document.getElementsByTagName("table")
//        nOtable = oTables.length;
//     for(i=0;i<nOtable;i++)
//         { oTables[i].className="table table-bordered table-striped"; }

$(document).ready(function(){
    $("table").removeClass("form data judulmenukecil dataprint").addClass("table table-bordered table-striped");
    if( $("table").hasClass("anti-table") ){
        $("table.anti-table").removeClass("table table-bordered table-striped");
    }
});
...
$(document).ready(function(){
    ...
    $(window).scroll(function(){
        ...
        var div_top = $('#aside-anchor').offset().top;
            if (window_top > div_top) {
                $('aside').addClass('sticky-left');
            } else {
                $('aside').removeClass('sticky-left');
            }
    });
